...
export class UserService {
  ...
  async addFavouriteCountry(id: String, countryId: string): Promise<User> {
    let criteria = { _id: id };
    const user = await UserModel.findOneAndUpdate(criteria, { $addToSet: { countries: countryId } },
      { new: true }).select(AppConstant.USER.DEFAULT.PROJECTION);
    return user;
  }
